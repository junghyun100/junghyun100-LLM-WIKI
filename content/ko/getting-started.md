...
> 📚 **공식 문서**: [Quartz 문서](https://quartz.jzhao.xyz/) · [Pagefind 검색](https://pagefind.app/) · [GitHub Pages 배포](https://docs.github.com/en/pages)
...
### 4. 사이드바/탐색기 설정 (Explorer)

`quartz.config.yaml`의 Explorer 플러그인 설정에서 구성:
...
Explorer는 `content/ko/` 디렉토리 구조를 자동으로 사이드바에 반영합니다. `autogenerate` 옵션은 Quartz에서 디렉토리 기반으로 자동 처리됩니다.

### 5. 로컬 확인

```bash
npx quartz build --serve      # 개발 서버
npx quartz build              # 프로덕션 빌드 검증
npx quartz build --serve -d public  # 빌드 결과 프리뷰
```
...
1. Node 22 환경에서 `npm ci --legacy-peer-deps` → `npx quartz build`
2. `public/` 디렉토리를 GitHub Pages 아티팩트로 업로드
3. `github-pages` 환경에 배포

**URL**: `https://junghyun100.github.io/junghyun100-LLM-WIKI/ko/`

### 수동 배포 (필요 시)

```bash
npx quartz build
# public/ 폴더 내용을 정적 호스팅에 업로드
```
...
# Quartz 플러그인 설치/업데이트
npx tsx ./quartz/plugins/loader/install-plugins.ts

# 빌드 캐시 클리어
rm -rf public .quartz-cache && npx quartz build
```
...
| 문제 | 해결 |
|------|------|
| 빌드 시 링크 에러 | `quartz.config.yaml` explorer 설정과 실제 파일 경로 일치 확인 |
| 프론트매터 검증 실패 | 필수 필드(`title`, `description`, `type` 등) 모두 포함 확인 |
| 탐색기에 안 보임 | `unlisted-pages` 플러그인 설정, `draft` 플래그 확인 |
| 한글 깨짐 | 파일 인코딩 UTF-8 확인, `locale: ko-KR` 설정 확인 |
| 포트 충돌 | `npx quartz build --serve --port 8082`로 포트 변경 |
...
4. `scripts/` — 자동화 스크립트 (lint-check, index-gen, ingest-helper)