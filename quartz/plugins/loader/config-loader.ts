...
export async function loadQuartzConfig(
  ...
  const layout = await loadQuartzLayout(undefined, manifests)
...
export async function loadQuartzLayout(
  ...
  // Use passed manifests or load them
  const manifestsMap = manifests ?? new Map<string, PluginManifest>()
  if (!manifests) {
    const enabledEntries = json.plugins.filter((e) => e.enabled)
    for (const entry of enabledEntries) {
      const manifest = await getManifest(entry.source)
      if (manifest) {
        manifestsMap.set(sourceKey(entry.source), manifest)
      }
    }
  }
  ...
  // Build entries: explicit layout from config + manifest defaults for multi-component plugins
  const allLayoutEntries: PluginJsonEntry[] = []

  // 1. Entries with explicit layout in config.yaml
  const entriesWithExplicitLayout = enabledEntries.filter((e) => e.layout)
  allLayoutEntries.push(...entriesWithExplicitLayout)

  // 2. Entries without explicit layout but with manifest default positions
  for (const entry of enabledEntries) {
    const manifest = manifestsMap.get(sourceKey(entry.source))
    if (!manifest?.components) continue

    // If entry has NO layout at all, create synthetic for all components with defaults
    if (!entry.layout) {
      const hasDefaults = Object.values(manifest.components).some(
        (c) => c.defaultPosition,
      )
      if (!hasDefaults) continue

      for (const [compName, compManifest] of Object.entries(manifest.components)) {
        if (!compManifest.defaultPosition) continue
        allLayoutEntries.push(makeSyntheticEntry(entry, compName, compManifest))
      }
      continue
    }

    // Entry HAS layout but only for some positions/components.
    // Create synthetic entries for components with defaults that are NOT explicitly placed.
    for (const [compName, compManifest] of Object.entries(manifest.components)) {
      if (!compManifest.defaultPosition) continue

      // Check if this component is explicitly placed in YAML
      // YAML has two formats:
      // 1. layout.position (singular string) - for plugin-level explicit position
      // 2. layout.positions (object mapping position -> component[]) - for per-page-type
      const explicitPosition = entry.layout.position
      const explicitPositions = entry.layout.positions?.[compManifest.defaultPosition] ?? []

      const isExplicitlyPlaced =
        explicitPosition === compManifest.defaultPosition || explicitPositions.includes(compName)

      if (!isExplicitlyPlaced) {
        allLayoutEntries.push(makeSyntheticEntry(entry, compName, compManifest))
      }
    }
  }

  // Helper to create synthetic layout entry
  function makeSyntheticEntry(
    entry: PluginJsonEntry,
    compName: string,
    compManifest: { defaultPosition: string; defaultPriority?: number },
  ): PluginJsonEntry & { _componentName?: string } {
    return {
      source: entry.source,
      enabled: true,
      options: entry.options,
      layout: {
        position: compManifest.defaultPosition,
        priority: compManifest.defaultPriority ?? 0,
      },
      _componentName: compName,
    }
  }
  ...
  const defaultLayout = buildLayoutForEntries(allLayoutEntries, layoutConfig, manifests)
  ...
  if (layoutConfig.byPageType) {
    for (const [pageType, override] of Object.entries(layoutConfig.byPageType)) {
      let filteredEntries = allLayoutEntries

      // Apply exclusions (by plugin name)
      ...
      const ptLayout = buildLayoutForEntries(filteredEntries, layoutConfig, manifests)

      // Apply position overrides (empty array = clear position, non-empty = explicit component placement)
      if (override.positions) {
        for (const [pos, components] of Object.entries(override.positions)) {
          if (!Array.isArray(components)) continue

          const key = pos as keyof FullPageLayout

          // Positions that expect a single component vs array
          const singleComponentPositions: Set<keyof FullPageLayout> = new Set(["head", "footer"])

          if (components.length === 0) {
            // Empty array: clear the position
            if (key in ptLayout) {
              if (singleComponentPositions.has(key)) {
                ;(ptLayout as Record<string, unknown>)[key] = undefined
              } else {
                ;(ptLayout as Record<string, unknown>)[key] = []
              }
            }
          } else {
            // Non-empty array: explicitly place these components
            const explicitComponents: QuartzComponent[] = []
            for (const compName of components) {
              // Look up component in registry (try exact, then PascalCase)
              let reg = componentRegistry.get(compName)
              if (!reg) {
                const pascalName = compName
                  .split("-")
                  .map((s) => s.charAt(0).toUpperCase() + s.slice(1))
                  .join("")
                reg = componentRegistry.get(pascalName)
              }
              if (!reg) {
                console.warn(
                  `Component "${compName}" declared in layout.byPageType.${pageType}.positions.${pos} not found in registry. Skipping.`,
                )
                continue
              }
              ...
              explicitComponents.push(component)
            }
            if (explicitComponents.length > 0) {
              if (singleComponentPositions.has(key)) {
                // For single-component positions (head, footer), take the first component
                ;(ptLayout as Record<string, unknown>)[key] = explicitComponents[0]
              } else {
                ;(ptLayout as Record<string, unknown>)[key] = explicitComponents
              }
            }
          }
        }
  ...
  // Add Head (built-in) and Footer (from component registry or buildLayoutForEntries result)
  const HeadModule = await import("../../components/Head")
  const head = HeadModule.default()

  // Footer: check if buildLayoutForEntries provided one via manifest default (e.g., SiteFooter)
  // First try the footer array from defaultLayout (populated by buildLayoutForEntries for synthetic entries)
  let footer: QuartzComponent | undefined
  if (defaultLayout.footer && defaultLayout.footer.length > 0) {
    footer = defaultLayout.footer[0] // Take first footer component
  } else {
    // Fallback: look for a "footer" plugin or "Footer"/"SiteFooter" component in registry
    ...
    if (footerEntry) {
      ...
      if (footerReg) {
        if (typeof footerReg.component === "function") {
    ...
    // Try our custom SiteFooter component
    if (!footer) {
      const siteFooterReg = componentRegistry.get("SiteFooter")
      if (siteFooterReg) {
        const footerEntry = json.plugins.find(
          (e) => e.enabled && e.source === "./local/llm-wiki-components",
        )
        if (footerEntry) {
          if (typeof siteFooterReg.component === "function") {
            const footerOverrides = componentRegistry.getOptionOverrides("SiteFooter")
            const opts = { ...footerEntry.options, ...footerOverrides }
            footer = componentRegistry.instantiate(
              siteFooterReg.component as QuartzComponentConstructor,
              Object.keys(opts).length > 0 ? opts : undefined,
            )
          } else {
            footer = siteFooterReg.component as QuartzComponent
          }
        }
      }
    }
...
function buildLayoutForEntries(
  ...
  for (const entry of entries) {
    ...
    // For synthetic entries (from manifest), use _componentName; otherwise use plugin name
    const name = (entry as PluginJsonEntry & { _componentName?: string })._componentName
      ? (entry as PluginJsonEntry & { _componentName?: string })._componentName!
  ...
  for (const [position, items] of Object.entries(positions)) {
    ...
    // Allow all FullPageLayout positions including header and footer
    const key = position as keyof FullPageLayout
